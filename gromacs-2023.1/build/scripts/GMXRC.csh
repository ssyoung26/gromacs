...
set tmppath = ""
foreach i ( `echo $DYLD_LIBRARY_PATH | sed "s/:/ /g"` )
  if ( "$i" != "$GMXLDLIB" ) then
    if ("${tmppath}" == "") then
      set tmppath = "$i"
    else
...
setenv DYLD_LIBRARY_PATH $tmppath
...
if ($?DYLD_LIBRARY_PATH) setenv DYLD_LIBRARY_PATH ":${DYLD_LIBRARY_PATH}"
if ($?PKG_CONFIG_PATH) setenv PKG_CONFIG_PATH ":${PKG_CONFIG_PATH}"

#path is never empty
setenv PATH ${GMXBIN}:${PATH}
setenv DYLD_LIBRARY_PATH ${GMXLDLIB}${DYLD_LIBRARY_PATH}
